<script lang="ts">
	...
	let phoneInvalid = $derived(
		phoneTouched && phone.length > 0 && phone.replace(/\s+/g, '').length < 10
	);
</script>
...
<div>
	...
	<div class="mb-4">
		<label class="block">
			...
			<input
				type="tel"
				value={phone}
				oninput={(e) => onUpdate('phone', e.currentTarget.value)}
		...
		{#if phoneInvalid}
			<p class="mt-1 text-xs text-red-500">Numéro trop court (min. 10 chiffres)</p>
		{/if}
	</div>
	...
	<div class="mb-4">
		...
		<div class="flex flex-wrap gap-2">
			<button
				type="button"
				class="flex items-center gap-2 border-2 px-4 py-2 text-sm transition-all duration-200
					{preferredContact === 'phone' ? 'border-primary bg-primary/5 font-medium' : 'border-gray-200 bg-white hover:border-primary/40'}"
			...
			<button
				type="button"
				class="flex items-center gap-2 border-2 px-4 py-2 text-sm transition-all duration-200
					{preferredContact === 'email' ? 'border-primary bg-primary/5 font-medium' : 'border-gray-200 bg-white hover:border-primary/40'}"
			...
			<button
				type="button"
				class="flex items-center gap-2 border-2 px-4 py-2 text-sm transition-all duration-200
					{preferredContact === 'any' ? 'border-primary bg-primary/5 font-medium' : 'border-gray-200 bg-white hover:border-primary/40'}"
