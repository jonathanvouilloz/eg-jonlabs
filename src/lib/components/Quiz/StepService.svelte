...
<div>
	...
	<div class="grid grid-cols-2 gap-3">
		{#each activeServices as service (service.id)}
			<button
				type="button"
				class="group relative aspect-[4/3] overflow-hidden border-2 text-left transition-all duration-200
			...
			>
				...
				{/if}
				<div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent transition-all duration-200 group-hover:from-black/60
					{selectedId === service.id ? 'from-black/60' : ''}"></div>
				<div class="absolute inset-x-0 bottom-0 p-5">
					<span class="text-xl font-semibold text-white drop-shadow-md">{service.label}</span>
				</div>
				{#if selectedId === service.id}
					<div class="absolute top-3 right-3 flex h-6 w-6 items-center justify-center bg-primary text-white">
